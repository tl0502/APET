// 系统托盘服务（#6 + #9 设置入口 + #35 ADR-021 P1 workspace 入口 + #33 P2 精简）
//
// 行为：
// - 启动时注册到 Windows 托盘（任务栏 ^ 展开后可见，用户可拖出常驻）
// - 左键单击 → 无操作；左键双击 → show workspace（VSCode-style "双击图标主面板"惯例）
// - 右键 → 弹菜单：显示/隐藏 / 工作台 / 番茄 / AOT / 退出（#33 phase E 精简：删 任务... + 设置...，迁入 workspace 后由 brand bar nav 入口）
// - 主窗 + chat/pomodoro/workspace CloseRequested 拦截在 lib.rs，改 hide
// - 唯一退出路径 = 托盘"退出"
//
// 设计要点：
...
/// #35 ADR-021 P1 workspace 主窗入口（settings/tasks panel 走 brand bar nav）。
...
pub fn setup(app: &AppHandle) -> tauri::Result<()> {
    ...
    // #28 follow-up 番茄独立窗：浮窗仅托盘"番茄..."手开（#33 phase E：删 pomodoro_start 自动 show）。
    let pomodoro_item = MenuItemBuilder::with_id(MENU_ID_POMODORO, "番茄...").build(app)?;
    // #35 ADR-021 P1 workspace 入口；settings/tasks panel 已迁入 workspace（#33 phase B/C）—— 托盘
    // 不再单独列"任务..."/"设置..."，用户走"工作台..." → brand bar nav 切类别。
