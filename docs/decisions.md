...
- **Updated 2026-05-24（chat 默认尺寸 380×480 → 640×480）**：B.3.c 落地后实测发现 chat 默认 380×480 在多会话列表 + composer 输入区共存时局促（列表过窄需横向滚 / composer 高度被挤）；提至 **640×480 默认 + min 480×400 / max 1000×800 可调范围**，实测体验流畅。[tauri.conf.json](../src-tauri/tauri.conf.json) `chat.width/height/minWidth/maxWidth/minHeight/maxHeight` 已同步；PRD §7.2.1 v1.2 已同步。其他 spoke 窗（tasks 420×600 / pomodoro 360×480）默认尺寸未变。
