...
- 让前端 `validatePersonaDraft` 覆盖 Rust 当前 blocking errors：`name.empty`、`identity.empty`、`personality.empty`、`capabilities.empty`、规则缺失。
...
规则校验继续保持前端更具体的 Do / Don't 诊断；Rust 当前将 Do 和 Don't 同空视为 blocking，并把单边缺失视为 warning。前端可以继续对 Do 缺失更严格，因为现有 UI 保存前会显示 diagnostics；这不影响后端一致性，且符合当前编辑体验。
...
- 塑形字段如果出现未知枚举值，Rust helper 使用保守默认文案，并保留原始值，避免保存崩溃；前端类型约束负责常规路径。
