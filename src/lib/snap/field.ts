...
// - ATTACH_ZONE 是吸附触发阈值，但用户在 (ATTACH_ZONE, FIELD_RADIUS] 区间也应感知到"磁场存在"
// - field intensity 渐进 [0,1]：FIELD_RADIUS 外为 0，ATTACH_ZONE 内（已进入吸附区）为 1
//
// 设计要点：
...
//
// 数值常量集中在 geometry.ts（#30 follow-up D 调整后 ATTACH=25 / FIELD_RADIUS=50）；
// 本文件文档保持符号化（避免常量变化时再次失真）。
 ...
 *  - distance ≤ ATTACH_ZONE：直接 1.0（已在吸附区，最强反馈）
 *  - ATTACH_ZONE < distance ≤ FIELD_RADIUS：渐进 0→1
 *  - distance > FIELD_RADIUS：0（远离）
 *
 *  渐进段映射：x ∈ (ATTACH_ZONE, FIELD_RADIUS] → intensity ∈ (0, 1) 线性外推。
...
export function fieldIntensityFromDistance(distance: number): number {
  ...
  // 在 [ATTACH_ZONE, FIELD_RADIUS] 区间线性映射到 [1, 0]
